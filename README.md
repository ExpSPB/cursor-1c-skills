...
Отдельно - справочники API прикладных конфигураций: первый такой скил `zup-hr-api-reference` покрывает кадровый учет 1С:ЗУП 3.1. Паттерн легко расширяется на ERP, УТ, Бухгалтерию - большой справочник выносится в `references/`, скил подгружает его в контекст только при срабатывании триггера (экономия токенов).
